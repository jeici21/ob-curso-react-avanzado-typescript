...
function App() {
  ...
  const hazBusqueda = () => {
    ...
    axios.get(url).then(r => setListado(r.data)).catch(e => console.error(e))
  }
  ...
  return (
    <div className="App">
      ...
      <input type="text" placeholder='Escribe el nombre de la radio' value={busqueda}
      ...
      <section aria-label='listado-emisoras'>
        {listado.map((emisora, i) => <div key={i}>{emisora.name} <IoPlay style={{ cursor: 'pointer' }}
          onClick={() => playRadio(emisora)} /></div>)}
      </section>
    </div>
  );
}
