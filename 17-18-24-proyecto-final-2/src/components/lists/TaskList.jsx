...
const TaskList = ({ showSettings, setShowSettings }) => {
  ...
  useEffect(() => getTasks().then(tasks => setTasklist([...tasks])).catch(e => console.error(e)), []);
  ...
  const addNewTask = () => {
    ...
    }).catch(e => console.error(e)).finally(() => setNewTask(""));
  };
  ...
  return (
    <>
      <header className="flex justify-between">
        ...
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="btn"
      ...
      <div className="my-4">
        <input className="shadow py-1 px-2 rounded-lg outline-none transition-all duration-300 focus:ring-2 mr-2 dark:bg-slate-700"
          value={newTask} onKeyDown={insertNewItemOnEnterKey} onChange={editNewItem} placeholder="New Task"
          type="text" />
          ...
          {tasklist.map((item, index) => (
            <motion.li initial={{ x: "100vw" }} animate={{ x: 0 }} key={index}>
              <label className="cursor-pointer">
                <input type="checkbox" onClick={() => toggleCompleteItem(item.id)} checked={item.completed}
                  // onClick={() => removeItem(index)}
                  onChange={() => { }} />
                <span className={`ml-2 text-gray-800 dark:text-gray-100 text-sm italic ${item.completed && "line-through"}`}>
                  {item.task}
                </span>
              </label>
            </motion.li>
