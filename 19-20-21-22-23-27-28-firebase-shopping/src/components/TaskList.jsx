...
const task = {
  title: "Este es el título", description: "Esta es la descripción"
};
...
const TaskList = () => {
  ...
  useEffect(() => initializeTasks(), []);

  return (
    <div>
      <h1 className="text-sky-700 font-semibold text-lg">Estás en la tasklist</h1>
      <div className="flex flex-col gap-4">
        <h2>Introduce una nueva tarea</h2>
        <input type="text" value={task.title} placeholder="Título"
          className="border shadow outline-none focus:ring ring-sky-200 rounded px-2 py-1 w-full"
          onChange={(e) => setTask({ ...task, title: e.target.value })} />
        <textarea type="text" rows={3} value={task.description} placeholder="Descripción"
          className="border shadow outline-none focus:ring ring-sky-200 rounded px-2 py-1 w-full"
        ...
        {tasks.map(task => (
          <div key={task.id} className="rounded-lg border border-sky-300 p-4 flex flex-col gap-2">
            ...
            <div className="flex justify-between">
              <button className="bg-sky-400 text-white py-1 px-2 rounded" onClick={() => editTask(task.id)}>
                Editar
              </button>
              <button className="bg-red-600 text-white py-1 px-2 rounded" onClick={() =>
